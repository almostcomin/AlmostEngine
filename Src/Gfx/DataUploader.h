...
public:
	...
	std::expected<nvrhi::EventQueryHandle, std::string> UploadBufferData(
		...
		st::Blob&& srcData, size_t dstStart = 0, int dstSize = -1, const char* opt_gpuMarker = nullptr);
	...
	std::expected<nvrhi::EventQueryHandle, std::string> UploadTextureData(
		...
		const st::IBlob& srcData, const nvrhi::TextureSubresourceSet& subresources, const char* opt_gpuMarker = nullptr);
