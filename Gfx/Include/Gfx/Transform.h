#pragma once

#include "Core/Math/glm_config.h"

namespace alm::gfx
{

class Transform
{
public:

	Transform() :
		m_Scale{ 1.f }, 
		m_Rotation{ glm::identity<glm::quat>() }, 
		m_Translation{ 0.f },
		m_Dirty{ false },
		m_Matrix{ glm::identity<float4x4>() }
	{}

	Transform(const float3& scale, const glm::quat rot, const float3 translation) :
		m_Scale(scale), 
		m_Rotation(rot), 
		m_Translation(translation), 
		m_Dirty{ true }
	{}

	// Init from an affine matrix. Warning: m must be in column-major order
	explicit Transform(const float4x4& m) :
		m_Scale{ GetScale(m) },
		m_Rotation{ GetRotation(m) },
		m_Translation{ GetTranslation(m) },
		m_Dirty{ true }
	{}

	const float3& GetScale() const
	{
		return m_Scale;
	}

	const glm::quat& GetRotation() const 
	{
		return m_Rotation;
	}

	const float3& GetTranslation() const
	{
		return m_Translation;
	}

	// Returns composed matrix. Warning: Column-major
	const float4x4& GetMatrix()
	{
		if (m_Dirty)
		{
			m_Matrix =
				glm::translate(glm::mat4(1.0f), m_Translation) *
				glm::mat4_cast(m_Rotation) *
				glm::scale(glm::mat4(1.0f), m_Scale);
			m_Dirty = false;
		}
		return m_Matrix;
	}

	Transform& SetScale(const float3& scale)
	{
		m_Scale = scale;
		m_Dirty = true;
		return *this;
	}

	Transform& SetRotation(const glm::quat& rot)
	{
		m_Rotation = rot;
		m_Dirty = true;
		return *this;
	}

	Transform& SetTranslation(const float3& t)
	{
		m_Translation = t;
		m_Dirty = true;
		return *this;
	}

	static float3 GetScale(const glm::mat4& m)
	{
		return {
			glm::length(float3(m[0])),
			glm::length(float3(m[1])),
			glm::length(float3(m[2]))};
	}

	static glm::quat GetRotation(const glm::mat4& m)
	{
		float3 scale = GetScale(m);
		glm::mat3 rotMat(
			float3(m[0]) / scale.x,
			float3(m[1]) / scale.y,
			float3(m[2]) / scale.z
		);
		return glm::quat_cast(rotMat);
	}

	static float3 GetTranslation(const glm::mat4& m)
	{
		return float3(m[3]);
	}

private:

	float3 m_Scale;
	glm::quat m_Rotation;
	float3 m_Translation;
	
	bool m_Dirty;
	float4x4 m_Matrix; // Column major
};

}