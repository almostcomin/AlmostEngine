	...
	bool Initialize() override
	{
		...
		alm::weak<alm::gfx::SceneHeightmap> sceneHeightmap;
		{
			...
			std::shared_ptr<alm::gfx::IHeightmapSource> dataSource;
			{
				...
				auto noiseSource = std::make_shared<alm::gfx::NoiseHeightmapSource>(params, 100.0f, 1.f, "NoiseHeightmap");
				...
				bool sourceOk = imageSource->Load("SpainHeightmap.png", 98.f, 1.f);
			...
			heightmap->Init(dataSource, mat,
				dataSource->InfiniteDataResolution() ? uint2{ 1024, 1042 } : dataSource->GetDataResolution(), 32);
